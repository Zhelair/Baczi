...
async function deduplicateRules() {
  ...
  for (const group of duplicateGroups) {
    ...
    if (delErr) {
      console.log(`  Error deleting duplicates for "${keep.pattern}": ${delErr.message}`)
    } else {
      console.log(`  Merged "${keep.pattern}" — kept id=${keep.id} (${keep.confidence}), removed ${idsToDelete.length}`)
